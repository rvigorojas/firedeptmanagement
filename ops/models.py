...
class ServiceAffected(models.Model):
    ...
    person_affected = models.ForeignKey(BasePerson, on_delete=models.CASCADE, related_name="services_involved")
...
class Service(models.Model):
    ...
    created_by = models.ForeignKey(Firefighter, on_delete=models.CASCADE, related_name='services_created', editable=False, null=True, blank=True)
...
class ServiceVehicle(models.Model):
    ...
    service = models.ForeignKey(Service, on_delete=models.CASCADE, related_name='vehicles', verbose_name=u"Servicio")
    lead = models.ForeignKey(Firefighter, on_delete=models.CASCADE, related_name='services_lead', verbose_name=u"Jefe de Servicio")
    crew = models.ManyToManyField(Firefighter, related_name="in_services", verbose_name=u"Tripulación", null=True, blank=True)
    driver = models.ForeignKey(Firefighter, on_delete=models.CASCADE, related_name="services_drove", verbose_name=u"Conductor", null=True, blank=True)
    vehicle = models.ForeignKey(Vehicle, on_delete=models.CASCADE, verbose_name=u"Unidad", null=True, blank=True)
...
class ServiceImage(models.Model):
    service = models.ForeignKey(Service, on_delete=models.CASCADE, related_name='images', verbose_name=u"Servicio")
    uploader = models.ForeignKey(Firefighter, on_delete=models.CASCADE)
...
class Arrest(models.Model):
    ...
    creation_date = models.DateTimeField(auto_now=True)
    created_by = models.ForeignKey(Firefighter, on_delete=models.CASCADE, related_name='arrests_created', editable=False, null=True, blank=True, verbose_name='Agregado por')
    arrested = models.ForeignKey(Firefighter, on_delete=models.CASCADE, related_name="arrests", verbose_name=u"Arrestado", null=True, blank=True)
    ...
    def __str__(self):
        return u"%s %s %s" % (str(self.date), str(self.minutes), str(self.arrested))
...
def notify_arrested(sender, instance, created, **kwargs):
    ...
    if created:
        ...
        content = u"%s te ha insertado un nuevo arresto:\n\n%s\n\n%s, este debe ser aprobado por operaciones  e Inspectoria para ser actualizado en tus arrestos" % (str(instance.created_by), str(instance), str(instance.description))
...
class ArrestPayment(models.Model):
    ...
    creation_date = models.DateTimeField(auto_now=True)
    created_by = models.ForeignKey(Firefighter, on_delete=models.CASCADE, related_name=u'arrest_payments_created', editable=False, null=True, blank=True, verbose_name='Agregado por')
    payer = models.ForeignKey(Firefighter, on_delete=models.CASCADE, related_name=u"arrests_payments", verbose_name=u"Persona que paga el arresto", null=True, blank=True)
    ...
    def __str__(self):
        return u"%s %s %s" % (str(self.start_time),  str(self.minutes), str(self.payer))
...
def notify_arrest_payment(sender, instance, created, **kwargs):
    ...
    if created:
        ...
        content = u"%s te ha insertado un nuevo pago de arresto:\n\n%s\n\nEste debe ser aprobado por operaciones para ser actualizado en tus arrestos" % (str(instance.created_by), str(instance))
